...
- **2026-09-26 P33–P36 收尾：清掉误提交的软链，并按独立审计修掉 3 处缺陷**
  ...
  **1) 误提交的软链（先处理）**：整理提交时 `git add -A` 把两条**自引用软链**
  `web/node_modules`、`server/.gocache` 提进了公开仓库 —— 根因是 `.gitignore` 里写的是
  `node_modules/`，而**带斜杠的规则只匹配目录**，软链（120000 blob）匹配不到。
  该分支当时尚未推送，所以直接重写历史把两条移除，并补上不带斜杠的兜底规则
  `node_modules` / `.gocache`。重写后逐项核对：旧 tip → 新 tip 的差异**只有这两条**，
  索引其余条目与提交信息完全一致（`refs/backup/pre-cleanup-branch` 与
  `.private/backups/pre-cleanup-*/` 留作对照）。

  **2) 独立审计发现并修掉的缺陷**（另一个 agent 只读审计，我逐条复核后修复）：

  | 缺陷 | 后果 | 修法 |
  | --- | --- | --- |
  | 单文件设永久把"已永久的文件"又算一次新增占用 | 配额用满时对同一文件重发请求被 **409** 拒掉（PUT 不幂等：超时重试、双标签页即中） | 已是永久则按 0 字节计 |
  | 服务层漏判 `status`（管理员能给回收站里的文件设永久） | 该文件不计入配额（配额只算 active），恢复后却突然变成永久 | 服务层显式再判一次 active |
  | 目录级配额不足的文案写死"涉及 1 个文件" | 200 个文件的目录被拒时告诉用户"涉及 1 个文件" | `PermanentizableStats` 返回真实计数 |
  | `PermanentUsageUnderPath` 死代码（全仓无调用） | —— | 删除 |

  **3) 新测试的变异验证**（`TestPermanentQuotaEdgeCases`，三个变异各自如期报错）：
  重新计入已永久字节 → 409「还需 1.0 MB」；去掉 active 校验 → 管理员拿到 **200**
  （文件在回收站里就变成了永久）；写死 1 个文件 → 文案「涉及 1 个文件」。
  ⚠️ **第 2 条的第一版断言是错的**：配额恰好用满时它被"配额不足"挡下、**因错误的原因变绿**，
  是变异测试把它暴露出来的。已改为先放宽配额、再断言 409 + 「已被删除」，并走真实恢复接口
  验"永久化没有被延后生效"。教训：计数/状态类的断言很容易假绿，必须做变异。

  **4) 文档纠错**（原文档有两处与实现不符，都已改）：
  - `design.md` 写的 `PreviewSizeBytes()` **不存在**，实际是 `Values.PreviewMaxSizeBytes()`
    / `Values.PreviewSizeAllowed`；
  - 原配额说明称"回收站里的永久文件几天内必被清掉、放大受 `trash_days` 约束"——
    **对"删目录"不成立**（见待办 1），已改为如实描述；
  - `/files` 的 `meta.title` 还是「全部文件」，而该页现在是"按人查看"（移动端顶栏会显示它）；
  - 补了「回滚注意」：`0004` 不可逆，退旧版镜像前**必须**先
    `UPDATE files SET expires_at = NOW() + INTERVAL <retention_days> DAY WHERE expires_at IS NULL`，
    否则旧代码把 NULL 扫进 `time.Time` 会让**所有**走 `scanFile` 的接口 500（不是跳过某一行）。

  **待办（本次刻意不改，均已写进 design.md 备查）**：
  1. **删目录进回收站的文件 `purge_at` 为 NULL**（`store.SoftDeleteFilesUnderPath`，main 上的
     既有 SQL），而 `ListPurgeable` 要求 `purge_at` 非空 ⇒ 这些行**永远不会被自动清理**；
     而"属主主动删除单个文件"是会设 `purge_at` 的（`MarkTrashed`），两条 SQL 口径不一致，
     design.md 第 4 节又写着"删目录时文件软删，语义与删文件一致"。
     后果：删目录能释放永久额度却不释放磁盘，这层放大不会自行收敛。
     **要单独确认是有意还是 bug 再动**——它改的是回收站保留策略，不属永久功能范围。
  2. **迁移 runner 无锁**（`store.migrate` 先查已应用版本、再执行 DDL、再 `INSERT`，既无
     `GET_LOCK` 也无 `INSERT IGNORE`）：多副本同时首次启动时，后到者会因
     `schema_migrations` 主键冲突 1062 直接启动失败。0001–0003 同样没加锁（0003 已经在
     `ALTER` 已有表了，不是只有本次才碰存量数据）。当前生产单副本，暂不影响。
  3. 配额校验是"读已用 → 判定 → 单条 UPDATE"，无事务/行锁，并发下可能略微超出上限
     （业务闸，不是安全边界）。
  4. `web/scripts/mock-api.mjs` 的目录级永久按 `owner_id` 粗匹配，与真接口的 `rel_path`
     前缀语义不等价 ⇒ 浏览器检查测不到递归正确性（由 Go 集成测试守）。
