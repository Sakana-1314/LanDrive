...
| `/files/mine` | 我的文件 | 仅自己的文件，可改名 / 删除；**文件夹导航**（面包屑、新建/改名/删除、进入子目录）；上传入口为**整页拖放**（拖到页面任意位置松手即传，**传到当前所在目录**）+ 工具条「上传文件」/「上传文件夹」按钮（移动端无拖拽操作）；**拖入或选择文件夹时按 `webkitGetAsEntry()` / `webkitRelativePath` 还原层级**，逐级建目录（同名复用）后把文件放进各自目录；队列在列表上方 |
